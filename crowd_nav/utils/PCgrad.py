...
class PCGrad():
    ...
    def _project_conflicting(self, grads, has_grads, shapes=None):
        shared = torch.stack(has_grads).prod(0).bool()  # & operation
        pc_grad, num_task = copy.deepcopy(grads), len(grads)  # [num_task,paramdim]
        for g_i in pc_grad:
        ...
        # merged_grad = torch.stack([g for g in pc_grad]).sum(dim=0)
        return merged_grad
