    ...
    def initializeWithAlternativeRelational(self, data):
        ...
        for j in range(human_num):
            if j == 0:
                ...
                o2h_obstacle_id = torch.cat((o2h_obstacle_id, torch.tensor(range(obstacle_num)) + robot_num + human_num), dim=0)
                o2h_human_id = torch.cat((o2h_human_id, torch.ones_like(torch.tensor(range(obstacle_num))) * i), dim=0)
